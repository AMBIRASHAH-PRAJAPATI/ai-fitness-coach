...
import { Day, FitnessPlan, FormData } from "@/lib/types";
...
export default function PlanPage() {
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      const newPlan = await fetchAndSavePlan(formData);
      setPlan(newPlan);
    } catch (err: any) {
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={regenerate} disabled={isRegenerating}>
            {isRegenerating ? (
      ...
      {isRegenerating ? (
        ...
        <p className="mt-4 text-muted-foreground text-sm">
          Hang tight! We're refreshing your fitness insights 💫
        </p>
      </div>      
      ) : (
